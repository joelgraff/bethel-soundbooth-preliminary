...
def build_dot(data, rankdir="LR", splines="polyline", diagram_id=None):
    ...
    lines = [
        ...
        f"  splines={splines};",
        # Moderate on both axes, deliberately. ranksep spreads the drawing
        # along the rank axis, so in LR it buys width — the axis that is
        # already long — and a wide setting turned the sheets into ribbons
        # (the ATEM sheet reached 5.4:1) while barely touching what it was
        # supposed to fix: the PC sheet's slope spread moved 34.0 to 31.2
        # degrees for an aspect ratio of 3.1 to 4.3. These values give
        # slightly better slope spread AND slightly smaller area than the
        # original 1.1/0.6 on every sheet, without the stretch.
        #
        # Do not reach for splines=ortho either. Re-tested on these sheets: it
        # is fully axis-aligned but drives lines through the extender boxes
        # and drops xlabels on node text, 20 self-overlaps against 0 here.
        "  nodesep=0.5;",
        "  ranksep=1.4;",
        "  newrank=true;",
        '  fontname="Helvetica";',
